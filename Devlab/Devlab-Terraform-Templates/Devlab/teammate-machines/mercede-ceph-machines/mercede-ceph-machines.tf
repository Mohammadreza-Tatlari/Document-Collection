variable "vm_base_config" {
  ...
  default = {
    ...
    name_prefix = "mersede-ceph-srv" #change this name base on your machine clustser  it is going to iterate through number of machines (example test-v-srv1, test-v-srv2, ...)
    cores = 4
    sockets = 2 
    memory = 8192
    ...
    ip_gw = "172.31.11.254"
    ip_start_offset = 53 #change this IP offset for each Collaborator Range IP  and it will be increased based on number of machines (example 172.31.11.51, 172.31.11.52, ...)
    ciuser = "ubuntu"
    cipassword = "Aa123456"
    sshkeys = "ssh-ed25519 zxxxxxx xx@xx.com" # add your ssh key to be used inside each machine
  }
}

variable "vm_count" {
  default = 3 # change this based on number of machines you need.
}
